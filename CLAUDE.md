...
**Project status: every v1-scope feature in the requirements doc is now implemented, tested, and documented.** Remaining work going forward is either (a) real third-party credentials arriving from the client — zero code changes needed, just set env vars — or (b) genuinely new feature requests, not gaps in this plan. If picking this up fresh, it's still worth doing one more line-by-line pass against the original doc text (reproduced near the top of this file) before assuming nothing else was missed — the two items above were caught only because the user asked directly, not because a systematic doc-diff was run.
