...
- **Settings-split gotcha (found in Phase 12)**: `config/settings/prod.py` had a **hardcoded** `CORS_ALLOW_ALL_ORIGINS = False` left over from Phase 0, predating `base.py`'s later env-driven `CORS_ALLOW_ALL_ORIGINS = env.bool(...)` toggle (added so Saad's Admin Web dev could unblock a CORS preflight before his exact origin was known — see below). Since `prod.py` does `from .base import *` and then reassigns the name unconditionally, it silently overrode the env var on every request in production — the Render env var showed `True` in the dashboard and `os.environ` genuinely had `"True"`, but `settings.CORS_ALLOW_ALL_ORIGINS` still evaluated `False` at runtime, discovered only by adding a temporary protected diagnostic endpoint that dumped both values side by side. Fixed by making `prod.py`'s line env-driven too (`env.bool("CORS_ALLOW_ALL_ORIGINS", default=False)`), same safe default, now actually overridable. **Lesson: when adding a new env-driven setting to `base.py`, grep `dev.py`/`prod.py`/`test.py` for a hardcoded override of that same name before assuming the env var takes effect** — `from .base import *` doesn't protect a name from being silently reassigned afterward in the same file.
